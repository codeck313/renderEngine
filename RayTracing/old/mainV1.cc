...
int main() {
  ...
  const int image_width = 400;          // 1920;
  ...
  for (int j = image_height - 1; j >= 0; --j) {
    ...
    for (int i = 0; i < image_width; ++i) {
      ...
      for (int s = 0; s < samples_per_pixel; ++s) {
        ...
        pixel_color += ray_color(r, world, max_depth, 0.35);
      }
      ...
      // std::cerr << static_cast<int>(256 * clamp(pixel_color.z(), 0.0, 0.999))
  ...
  std::cerr << "\nDone in " << duration.count() << " minutes\n";
}
